...
use crate::sync::lock::{LockMode, Unlock};
...
impl CondVar {
    ...
    fn wait_internal<T: ?Sized, B: Backend, M: LockMode + Unlock<B>>(
        &self,
        wait_state: c_int,
        guard: &mut Guard<'_, T, B, M>,
    ...
    /// spuriously.
    pub fn wait<T: ?Sized, B: Backend, M: LockMode + Unlock<B>>(&self, guard: &mut Guard<'_, T, B, M>) {
    ...
    pub fn wait_interruptible<T: ?Sized, B: Backend, M: LockMode + Unlock<B>>(
        &self,
        guard: &mut Guard<'_, T, B, M>,
    ) -> bool {
    ...
    pub fn wait_interruptible_timeout<T: ?Sized, B: Backend, M: LockMode + Unlock<B>>(
        &self,
        guard: &mut Guard<'_, T, B, M>,
        jiffies: Jiffies,
